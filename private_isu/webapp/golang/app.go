...
func getPosts(w http.ResponseWriter, r *http.Request) {
	...
	err = db.Select(
		&postHTMLs,
		"SELECT `html` FROM `posts` FORCE INDEX (`created_at`) INNER JOIN `users` FORCE INDEX (`idx_id_del_flg`) ON `users`.`id` = `posts`.`user_id` AND `users`.`del_flg` = 0 INNER JOIN `post_htmls` ON `posts`.`id` = `post_htmls`.`post_id` WHERE `posts`.`created_at` <= ? ORDER BY `posts`.`created_at` DESC LIMIT ?",
